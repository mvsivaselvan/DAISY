function [T, X, Xp] = ...
    SolveTemplate2StructureDynamics(di_S1, phii_S1, ...
...
% Dynamic analysis of assembled structure
...
% tspan = time span to compute solution over
% OUTPUTS
% T - time
% X - solution 
% Xp = time derivatives of the solution 
...
% define initial Dofs asscoiated to Joints 1 and 2 
Ji_1 = [di_S1; phii_S1];
Ji_2 = [di_S2; phii_S2];
...
sol = ode15i(@odefun, [0 tspan(end)], x0, xp0, opt);
T = tspan';
[X, Xp] = deval(sol, T);
X = X'; % transposed so that it is consisted with lsim
Xp = Xp';
